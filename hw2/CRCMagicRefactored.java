...
public class CRCMagicRefactored {
    public static void main(String[] args) {
        ...
        ArrayList<Integer> myMessageMagic = myMessage;
        int mesLen = myMessageMagic.size();

        // добавляем преобразованное CRC-значение, которое получили
        for (int i = 0; i < result3.size(); i++) {
            myMessage.set(mesLen - 32 + i, result3.get(i));
        }

        // снова добавляем нули, как и раньше
        for (int i = 0; i < 32; i++) {
            myMessageMagic.add(0);
        }

        ArrayList<Integer> resultMagic = countRest32(myMessageMagic, returnPoly());
        System.out.println("Magic result: " + resultMagic);
        System.out.println("Magic result in 10th format: " + turn2to10(resultMagic));
    }
